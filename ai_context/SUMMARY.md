# RPG Integration Example: ai_context в графе проекта
# Демонстрация варианта 2 - расширение rpg_py_accountant.yaml

# Пример: как добавить ai_context секцию к узлу use case в RPG графе

nodes:
  - id: N-UC-POST-TX
    name: "AsyncPostTransaction"
    type: use_case
    layer: application
    status: implemented
    
    # Существующие поля RPG (не меняются)
    dependencies:
      - N-PORT-UOW
      - N-PORT-CLOCK
      - N-DOM-TRANSACTION
      - N-DOM-ENTRY-LINE
      - N-DTO-ENTRY-LINE
      - N-DTO-TRANSACTION
    
    implementation:
      file: src/py_accountant/application/use_cases_async/ledger.py
      class: AsyncPostTransaction
      lines_of_code: 85
    
    tests:
      - tests/unit/application/use_cases_async/test_ledger.py::test_post_transaction_success
      - tests/unit/application/use_cases_async/test_ledger.py::test_post_transaction_unbalanced
      - tests/integration/ledger/test_post_transaction_integration.py
    
    # 🆕 AI Context - компактная информация для моделей
    ai_context:
      # Контракты входов/выходов
      contracts:
        constructor:
          - name: uow
            type: AsyncUnitOfWork
            protocol: true
          - name: clock
            type: Clock
            protocol: true
        
        input:
          - name: lines
            type: list[EntryLineDTO]
            min_length: 2
            schema:
              full_name: str  # "ASSET:CASH_USD"
              debit: Decimal  # quantized(2)
              credit: Decimal # quantized(2)
          - name: memo
            type: str
          - name: meta
            type: dict | None
            optional: true
        
        output:
          type: TransactionDTO
          schema:
            transaction_id: int
            posted_at: datetime  # UTC
            memo: str
            lines: list[EntryLineDTO]
            meta: dict | None
        
        exceptions:
          - ValidationError: "Double-entry invariant violated"
          - ValueError: "Account not found"
          - IntegrityError: "Idempotency key duplicate"
      
      # Data flow (компактно)
      dataflow:
        - step: validate
          action: "Check sum(debits) == sum(credits)"
        - step: load_accounts
          action: "uow.accounts.get_by_full_name() for each line"
        - step: create_domain
          action: "Transaction.create(lines, memo, posted_at, meta)"
        - step: persist
          action: "uow.transactions.add() + uow.commit()"
        - step: return_dto
          action: "Convert Transaction → TransactionDTO"
      
      # Инварианты
      invariants:
        - "sum(line.debit) == sum(line.credit)"
        - "len(lines) >= 2"
        - "all(line.debit >= 0 and line.credit >= 0)"
        - "All amounts quantized to 2 decimals"
      
      # Паттерн использования
      usage_pattern: |
        async with async_uow_factory() as uow:
            uc = AsyncPostTransaction(uow, SystemClock())
            result = await uc(
                lines=[EntryLineDTO(...), EntryLineDTO(...)],
                memo="Transaction memo",
                meta={"idempotency_key": "tx-001"}
            )
      
      # Типичные ошибки
      common_errors:
        - error: "Unbalanced transaction"
          cause: "sum(debits) != sum(credits)"
          fix: "Ensure double-entry invariant"
        - error: "Account not found"
          cause: "full_name doesn't exist"
          fix: "Create account first with AsyncCreateAccount"

---

# Преимущества этого подхода:

# 1. Единый источник истины
#    - Граф зависимостей + контракты в одном файле
#    - Топологический порядок реализации сразу виден

# 2. Компактность
#    - Только ключевая информация
#    - Без "шума" документации для человека

# 3. Интеграция с RPG
#    - tools/rpg может генерировать ai_context
#    - Автоматическая валидация против кода

# 4. Модульная загрузка
#    - Модель может запросить только нужный узел
#    - Граф показывает зависимости для полного контекста

---

# Пример запроса модели:

# "Implement AsyncCreateAccount use case"

# 1. Загрузить узел N-UC-CREATE-ACCOUNT из графа
# 2. Посмотреть dependencies → загрузить N-PORT-UOW, N-DOM-ACCOUNT
# 3. Посмотреть ai_context → увидеть contracts, dataflow, invariants
# 4. Найти похожий узел (N-UC-CREATE-CURRENCY) → скопировать паттерн

# Итого: ~200-300 строк контекста вместо 10,614

