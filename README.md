...
> Важно: Модуль `py_fledger` помечен DEPRECATED. План удаления: через 2 итерации (см. RPG NS план). Используйте use cases и CLI. Последний шаг миграции описан ниже.
...
## Миграция с py_fledger.Book
Раздел содержит рекомендации перехода на use cases и CLI. Планируемая дата удаления py_fledger: +2 итерации с текущего релиза. Убедитесь, что вы используете новые команды и DTO. Команда `diagnostics:parity-report` помогает верифицировать паритет на ваших сценариях.
...
- `diagnostics:parity-report` для сравнения результатов между legacy `py_fledger.Book` и новым движком.

Статус: py_fledger.Book DEPRECATED (NS9). Новая функциональность полностью покрывает сценарии.

## Диагностика паритета
Добавлена CLI команда `diagnostics:parity-report` для сравнения результатов между legacy `py_fledger.Book` и новым движком (use cases). Пока legacy присутствует, команда строит JSON отчёт по нескольким сценариям.

Пример:

```bash
poetry run python -m presentation.cli.main diagnostics:parity-report --json
```

Вывод: JSON со статусом matched/diverged по каждому сценарию, расхождения (балансы по счетам, base_total trading, количество транзакций) и список `rounding_deltas` (отклонения > tolerance).

Tolerance можно настроить: `--tolerance 0.01`.

Если legacy недоступен, сценарии будут помечены как `skipped`.

## История курсов (ephemeral)
Добавлена CLI команда `diagnostics:rates-history` для просмотра последних N наблюдений курсов в рамках процесса. История хранится в памяти и очищается при завершении процесса.

```bash
poetry run python -m presentation.cli.main currency:add USD
poetry run python -m presentation.cli.main currency:add EUR
poetry run python -m presentation.cli.main fx:update EUR 1.2345
poetry run python -m presentation.cli.main fx:update EUR 1.2400
poetry run python -m presentation.cli.main diagnostics:rates-history --json
```

Настройка длины истории: переменная окружения `RATE_HISTORY_LEN` (по умолчанию 10). Ограничение: нет персистентного аудита.

## Индексы и производительность
Добавлены индексы для ускорения выборок леджера:
- journals(occurred_at)
- transaction_lines(account_full_name)
- transaction_lines(currency_code)
- transaction_lines(account_full_name, journal_id)

Миграция: Alembic ревизия `0003_add_performance_indexes` добавляет индексы с upgrade/downgrade.
...
Перфоманс-дымовой тест:
```bash
poetry run pytest tests/perf/test_cli_perf.py -q -k post_1000
```
