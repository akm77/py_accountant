...
- Примеры находок Ruff:
  - **Лямбды `l` (`E741`)** — десятки случаев в application/tests.
...
- **Рекомендация** — применить `ruff --fix` к импортам и простым правилам, затем вручную устранить сложные случаи (лямбды, тестовые assert). Подключить Ruff к CI.
