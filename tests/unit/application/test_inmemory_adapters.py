...
def test_inmemory_transaction_repo_balance() -> None:
    ...
    bal = repo.aggregate_trading_balance(as_of=now)
    assert bal.lines[0].total_debit == Decimal("25.00")
    assert bal.lines[0].total_credit == Decimal("25.00")
...
def test_inmemory_uow_basic() -> None:
    ...
    uow.commit()  # no-op
    bal = uow.transactions.aggregate_trading_balance(as_of=now)
    assert bal.lines[0].currency_code == "EUR"
